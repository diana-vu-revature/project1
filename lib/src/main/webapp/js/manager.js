  ...
  if(pendingArray != null) {
    if(pendingArray.length > 0) {
      pendingArray.forEach(pendingReim => {
        ...
        pendingTable.appendChild(denyTd);
...
function approveReim(id) {
  ...
  let pendingReimTr = document.getElementById(id);
  pendingReimTr.remove();
}
